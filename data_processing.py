...
class DataProcessing:
    ...
    def general_info(self):
        ...
        max_interspace = max([np.shape(self.years[key])[0] - 1 for key in self.data])
        min_interspace = min([np.shape(self.years[key])[0] - 1 for key in self.data])
    ...
    def missing_values_percentage(self, station):
        a = np.sum(np.isnan(self.data[station]))
        ll = np.shape(self.data[station])
    ...
    def replace_missing_median(self, station):
        ...
        Function changes global variable self.data for given station!!"""
        shape = np.shape(self.data[station])
        for i in range(shape[0]):
            for j in range(shape[1]):
                if np.isnan(self.data[station][i, j]):
                    self.data[station][i, j] = np.nanmedian(self.data[station][:, j])
    ...
    def choose_stations(self, min_year, min_mis_val_per=50):
        ...
        stations = []
        for station in [key for key in self.years if np.shape(self.data[key])[0] >= min_year + 1]:
    ...
    def stationarity_test(self, station):
        ...
        noy = np.shape(self.data[station])[0]
        n1 = int(noy / 2)
        n2 = noy - n1
    ...
    def ratio_test(self, station, plotting=False):
        ...
        t0 = t0s[0]
        length = np.shape(self.data[station])[0]
...
if __name__ == '__main__':
    ...
    print(s2.pettit_test(1, plotting=True))
